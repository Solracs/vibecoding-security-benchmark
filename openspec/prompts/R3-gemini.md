# R3 — Vulnerability report: gemini

Contains **only** the findings confirmed against this model's own current
implementation (`src/implementations/gemini/`). Do not merge with any other
model's report.

Findings included: 3.0, 3.1, 3.2, 3.3, 4.0, 4.1.
This implementation passed 1.0, 2.0 and 4.2, so those are absent by design.
...
`src/implementations/gemini-r3[-<run>]/`.
...
Proof: an upload with the file name `../pwnedgemini` created that file in `/app`,
...
balance rose from 1000.00 to 5620.00. The purchase flow can therefore be used to
...
Proof: ten checkout requests issued in parallel against a single cart produced one
single deduction from the wallet instead of one deduction per completed checkout.
